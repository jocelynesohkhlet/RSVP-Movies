...
/* 
The tables in our schema contain the follwing number of rows :
...
SELECT Sum(CASE
       ...
       Sum(CASE
             ...
             ELSE 0
           END) AS production_comapny_nulls
FROM   movie; 
	 
     
-- columns with the name country, world_wide_gross_income, languages and production_company from movie table have null values.
...
-- Comapring to other years, 2017 has the most movies released with 3052 movies.

-- Number of movies released each month 

SELECT Month(date_published) AS mon_num,
       Count(id)             AS number_of_movies
FROM   movie
GROUP  BY Month(date_published)
ORDER  BY number_of_movies DESC;

-- MARCH has the higgest movies released with 824 movies and  December has the lowest with 438 movies.
...
-- 1059 movies were released in USA or India in 2019.
...
-- Drama has the higgest number of movies produced overall with 4285
...
-- Movies of genre 'Action" has higgest average  duration with 112.88 mins and 'Drama' genre has average duration of 106.77 mins.
...
-- Thriller genre is Ranked 3rd in terms of the number of movies produced.
...
FROM   (SELECT production_company,
        ...
        GROUP  BY m.production_company) AS company_ranking
WHERE  prod_company_rank = 1; 


-- Dream Warrior Pictures and National Theatre Live has produced the most number of hit movies (average rating > 8)
...
-- Drama genre has the higgest movies released in each genre during March 2017 in the USA had more than 1,000 votes with 16 movies.
...
SELECT Sum(CASE
           ...
           END) AS german_votes_count,
       Sum(CASE
             WHEN Lower(m.languages) LIKE '%Italian%' THEN r.total_votes
           END) AS 'Italian votes count'
FROM   movie AS m
       INNER JOIN ratings AS r
               ON m.id = r.movie_id; 


-- German movies do get more votes than Italian movies.
...
-- Except for id and name, every other column in the names table has null values.
...
ON         m.id = r.movie_id,
           top_3_genre
WHERE      avg_rating > 8
AND        g.genre IN (top_3_genre.gnr)
GROUP BY   NAME
...
-- The top 2 actors who have at least a median rating of 8 are Mammootty and Mohanlal.
...
FROM     prod_comp_votes 
LIMIT 3;

-- Based on the number of votes recieved the top 3 production house are Marvel Studios, Twentieth Century Fox and Warner Bros..
...
-- Vijay Sethupathi tops the list with average rating 8.42.
...
FROM     indian_actress limit 5;

-- Taapsee Pannu tops the list with average rating of 7.74. The other 4 are Kriti Sanon, Divya Dutta, Shraddha Kapoor and Kriti Kharbanda.
...
FROM   (
                  SELECT     g.genre,
                             ...
                             CASE
                                        ...
                                        WHEN worlwide_gross_income LIKE "INR%" THEN round(CONVERT(replace(trim(worlwide_gross_income), "INR ",""), unsigned int) / 80)
                             END AS worldwide_gross_income, -- Removed the "$" sign and converted values that were "INR" to "$" at the rate of 80
                             rank() OVER (partition BY g.genre, year ORDER BY
                             CASE
                                        WHEN worlwide_gross_income LIKE "$%" THEN CONVERT(replace(trim(worlwide_gross_income), "$ ",""), unsigned int)
                                        WHEN worlwide_gross_income LIKE "INR%" THEN round(CONVERT(replace(trim(worlwide_gross_income), "INR ",""), unsigned int) / 80)
                  ...
                  ON         g.movie_id = m.id,
                             top_3_genre
                  WHERE      g.genre IN (top_3_genre.genre)) AS top_genre_movie_ranking
WHERE  movie_rank <= 5;
...
-- The top 3 actresses based on number of Super Hit movies (average rating >8) in drama genre are Sangeetha Bhat, Fatmire Sahiti and Adriana Matoshi
...
SELECT   director_id,
         director_name,
         Count(director_id)           AS number_of_movies,
...
-- Based on number of movies the director Andrew Jones tops the list followed by A.L. Vijay.